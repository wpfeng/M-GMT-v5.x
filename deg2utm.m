function  [x,y,utmzone] = deg2utm(Lat,Lon,zoneutm)
% -------------------------------------------------------------------------
...
if nargin<3 
   zoneutm = [];
   %[x0,y0,zoneutm] = deg2utm(mean(Lat(:)),mean(Lon(:)),[]);
end
%
%
n1=length(Lat);
...
x=zeros(n1,1);
y=zeros(n1,1);
if isempty(zoneutm)~=1
   zoneutm = MCM_rmspace(zoneutm);
   zoneutm = [zoneutm(1:length(zoneutm)-1),' ',zoneutm(end)];
   utmzone(n1,:) = zoneutm;
   tmp   = textscan(zoneutm,'%f %s',1);
   Huso  = tmp{1};
   Letra = tmp{2};
   Letra = Letra{1};
end
